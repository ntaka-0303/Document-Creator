...
1. **情報収集**:
   - 対象ドラフトの本文は、すでに入力として与えられているテキストをそのまま使用してください（ドラフト用に `read_file` を呼び出す必要はありません）。
   - 評価基準ファイルについては、`read_file` ツールを使用して `docs/<doc_type>/definitions/evaluation.md` を読み込んでください。
2. **評価実行**: ドラフト本文と評価基準ファイルの内容、および以下のタスクに従って評価を行ってください。
