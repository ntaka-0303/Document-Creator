...
各ドラフターから受け取った本文は、**ファイルには保存せず**、会話（メモリ）上で管理してください。

- 例として、次のような配列またはマッピングを内部的に保持します：
  - `drafts = [{ id: 1, style: structured, content: <本文> }, { id: 2, style: logical, content: <本文> }, ...]`
- 以降の評価フェーズ・再ドラフト生成フェーズ・最終化フェーズでは、この `drafts` に格納された本文を参照します。
- デバッグ目的などで手動で Write ツールを使って保存することは構いませんが、**標準フローでは中間ドラフトをファイル保存しない**ものとします。
