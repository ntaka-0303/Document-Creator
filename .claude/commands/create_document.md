...
このステップは**任意**ですが、特に通常モードでの実行時に有効です（高品質モードでは原本も積極的に参照して構いません）。

### 3.7. 高速モード / 通常モード / 高品質モードの選択

ユーザーに対して、以下の 3 つのモードのどれで実行するかを確認します（指定がない場合は**通常モード**をデフォルトとします）。

- **高速モード（新規）**
  - ドラフター数: **1つのみ**（`doc-drafter-structured` 固定）。
  - フロー: Step1（初回ドラフト）→ Step2（評価）→ Step3（フィードバック付き再ドラフト）までを**各1回のみ**実行します。
  - Step3 では、Step2 の評価結果・改善提案を取り込みつつ、**ループは行わず 1 回だけ再ドラフトを生成**します。
  - **Step4（最終化）は実施せず**、Step3 で生成された修正版ドラフトを「最終版」として保存・ユーザーに返します（最終版ファイルの保存パス規則は他モードと同じ）。
  - 用途: とにかく早く、評価結果を踏まえた 1 回きりの改善版が欲しい場合（統合作業よりも速度を優先）。
- **通常モード（旧「軽量モード」）**
  - ドラフター数: 原則 **2つまで**（`doc-drafter-structured` + doc_type に応じた 1 スタイル）。
  - フロー: Step1（初回ドラフト）→ Step2（評価）→ Step4（最終化）を実行し、**Step3（フィードバック付き再ドラフト生成）は実施しません**（＝フィードバック付きループは不要）。
  - 評価結果は、Step4 でのドラフト統合・採用判断のための情報としてのみ用います。
- **高品質モード**
  - ドラフター数: 従来通り **2〜3つ** を使用。
  - フロー: Step1〜Step4 をフルに用い、すべてのドラフトが高スコア `pass` になるまで、またはユーザーが指定した最大ループ回数に達するまで **Step3 を含めたループ**を実行します。

モードと選択したドラフターの組み合わせは、会話の中で「モード: 高速 / 通常 / 高品質」「使用ドラフター: structured + logical など」として明示します。
