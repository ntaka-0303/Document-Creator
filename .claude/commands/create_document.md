...
ただし、すべてのドラフトが高スコア（例: 4.5以上）で `pass` している場合、ユーザーの判断で Step3 をスキップして Step4（最終化）に進むことも可能です。
...
* **修正済みドラフトファイル**（すべて、@ 記法で参照）
  * `@docs/<doc_type>/output/draft/draft-1.md`
  * `@docs/<doc_type>/output/draft/draft-2.md`
  * `@docs/<doc_type>/output/draft/draft-3.md`
  * ...
* **各ドラフトの評価結果ファイル**（すべて、@ 記法で参照）
  * `@docs/<doc_type>/output/draft/eval-1.md`
  * `@docs/<doc_type>/output/draft/eval-2.md`
  * `@docs/<doc_type>/output/draft/eval-3.md`
  * ...
* **評価基準ファイル**
  * `@docs/<doc_type>/definitions/evaluation.md`
* **テンプレートファイル**
...
最終化エージェントから受け取った最終版ドキュメント本文を、**Write ツールを使用**して以下のパスに保存します：
