...
自動作成されたファイルをクリアする場合は、`/clear_outputs` コマンドを使用します：
...
- **番号（1-5）を入力**すると、その番号に対応する `docs/<doc_type>/output` ディレクトリ内のファイルが削除されます
- **`all` を入力**すると、すべてのドキュメント種別の `output` ディレクトリ内のファイルが削除されます
- 削除前に確認プロンプトが表示され、`yes` と入力した場合のみ削除が実行されます

**注意**: 
- `output` ディレクトリ自体は削除されません（空のディレクトリとして残ります）
- `definitions/` 配下のファイル（テンプレートや評価基準）は削除されません
- `docs/00_inputs/` 配下のファイルは削除されません
...
`/create_document` コマンドが以下のプロセスを自動的に実行します（実際にどこまで実行するか・どこをループするかはモードによって異なります）。
...
7. **Step3: フィードバック付き再ドラフト生成**: 各ドラフターがフィードバックをもとにドラフトを修正（モードによってはスキップ、または 1 回のみ実行）
8. **再評価**: 修正されたドラフトを再度評価（高品質モードで主に利用）
9. **Step4: 最終化**: 1つ以上のドラフトが合格した場合、最終化エージェントが複数のドラフトを統合して最終版を作成（高速モードでは通常スキップ）
10. **最終出力**: 最終版を `docs/<doc_type>/output/<テンプレート名>_<YYYYMMDD>.md` に保存

**注意**: 高品質モードでは、すべてのドラフトが不合格の場合に Step3 を繰り返すことができます（最大2〜3回を推奨）。高速モード・通常モードではフィードバック付きループは行いません。

### 実行モード

DocumentCreator は、ニーズに応じて 3 つの実行モードをサポートします（指定がない場合は通常モードが推奨されます）。

- **高速モード**
  - ドラフター: `doc-drafter-structured` のみ（1本）。
  - フロー: Step1（ドラフト生成）→ Step2（評価）→ Step3（フィードバック付き再ドラフト）を各 1 回だけ実行し、Step3 で生成したドラフトをそのまま最終版として保存します（Step4 の統合処理は行いません）。
  - 用途: なるべく早く、評価結果を 1 回だけ反映したドラフトが欲しい場合。
- **通常モード**
  - ドラフター: 原則 2 本まで（`doc-drafter-structured` + doc_type に応じた 1 スタイル）。
  - フロー: Step1（ドラフト生成）→ Step2（評価）→ Step4（最終化）を実行し、Step3（フィードバック付き再ドラフト生成）は行いません。
  - 用途: 標準的な品質と処理時間のバランスを取りたい場合（複数ドラフトからの統合は行う）。
- **高品質モード**
  - ドラフター: 2〜3 本を使用し、複数のスタイルでドラフトを生成。
  - フロー: Step1〜Step4 をフルに利用し、必要に応じて Step3（フィードバック付き再ドラフト生成）と再評価を 2〜3 回まで繰り返します。
  - 用途: 品質を最重視し、評価基準を十分に満たすまで改善と統合を繰り返したい場合。
