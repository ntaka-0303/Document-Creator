...
**すべての応答は必ず日本語で行うこと。** 英語での応答は禁止です。

## コマンド実行時の振る舞い（必須）

### `/create_document` コマンド実行時

**重要**: `/create_document` コマンドが実行された場合、以下を厳守してください：

1. **説明や挨拶は一切不要** - プロジェクトの説明、アーキテクチャの紹介、オプションの提示などは行わない
2. **即座にワークフローを開始** - コマンド実行後は、直接ドキュメント作成ワークフローを開始する
3. **最初のアクションはドキュメント種別の選択** - 以下の形式で番号選択を促す：
...
**禁止事項**:
- 「お手伝いさせていただきます」「何がお手伝いできますか？」などの挨拶
- システムの仕組みや選択肢の説明
- 「以下のようなオプションがあります」などの案内
- ワークフロー開始前の確認や質問
...
- **通常モード**
  - ドラフター: 原則 2 本まで（`doc-drafter-structured` + doc_type に応じた 1 スタイル）。**必ず2つのドラフターを使用し、1本のみを採用することは禁止**。
...
特に通常モードでは以下を厳守：
- Step1で**必ず2つのドラフターで並列ドラフト生成**を実行（1本のみの生成は禁止）
- Step2で**必ず全ドラフトを評価**（評価プロセスの省略は禁止）
- Step4で**必ず最終化エージェントで複数ドラフトを統合**（ドラフト1本をそのまま最終版にするのは禁止）
