...
3. **並列ドラフト生成** → 各ドラフターを呼び出し（独立実行）
4. **ファイル保存** → Write ツールで `docs/<doc_type>/output/draft/` に保存
5. **評価** → doc-evaluator で各ドラフトを評価
6. **判定と改善ループ** → pass なら最終版保存、fail なら再生成（最大2-3回）
7. **最終版保存** → Write ツールで `docs/<doc_type>/output/` に保存
